...
vlc_plugin_begin ()
        ...
        set_capability( VLC_CAP_INTERFACE, 0, Open_LuaIntf, Close_LuaIntf )

    add_submodule ()
        add_shortcut( "luahttp", "http" )
        set_description( N_("Lua HTTP") )
        set_capability( VLC_CAP_INTERFACE, 0, Open_LuaHTTP, Close_LuaIntf )

    add_submodule ()
        set_description( N_("Lua Telnet") )
        add_shortcut( "luatelnet", "telnet" )
        set_capability( VLC_CAP_INTERFACE, 0, Open_LuaTelnet, Close_LuaIntf )

    add_submodule ()
        set_description( N_( "Lua Meta Fetcher" ) )
        set_help( N_("Fetch meta data using lua scripts") )
        set_capability( VLC_CAP_META_FETCHER, 10, FetchMeta, NULL )

    add_submodule ()
        set_description( N_( "Lua Meta Reader" ) )
        set_help( N_("Read meta data using lua scripts") )
        set_capability( VLC_CAP_META_READER, 10, ReadMeta, NULL )

    add_submodule ()
        add_shortcut( "luaplaylist" )
        set_description( N_("Lua Playlist") )
        set_help( N_("Lua Playlist Parser Interface") )
        set_capability( VLC_CAP_STREAM_FILTER, 302, Import_LuaPlaylist, Close_LuaPlaylist )

    add_submodule ()
        set_description( N_( "Lua Art" ) )
        set_help( N_("Fetch artwork using lua scripts") )
        set_capability( VLC_CAP_ART_FINDER, 10, FindArt, NULL )

    add_submodule ()
        set_shortname( N_("Lua Extension") )
        add_shortcut( "luaextension" )
        set_capability( VLC_CAP_EXTENSION, 1, Open_Extension, Close_Extension )

    add_submodule ()
        set_description( N_("Lua SD Module") )
        add_shortcut( "luasd" )
        set_capability( VLC_CAP_SERVICES_DISCOVERY, 0, Open_LuaSD, Close_LuaSD )
