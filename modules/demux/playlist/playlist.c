...
vlc_plugin_begin ()
    ...
    add_submodule ()
        ...
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_M3U, NULL )
    add_submodule ()
        set_description( N_("RAM playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_RAM, NULL )
    add_submodule ()
        set_description( N_("PLS playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_PLS, NULL )
    add_submodule ()
        set_description( N_("B4S playlist") )
        add_shortcut( "shout-b4s" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_B4S, NULL )
    add_submodule ()
        set_description( N_("DVB playlist") )
        add_shortcut( "dvb" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_DVB, NULL )
    add_submodule ()
        set_description( N_("Podcast") )
        add_shortcut( "podcast" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_podcast, NULL )
    add_submodule ()
        set_description( N_("XSPF playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_xspf, Close_xspf )
    add_submodule ()
        set_description( N_("ASX playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_ASX, NULL )
    add_submodule ()
        set_description( N_("Kasenna MediaBase") )
        add_shortcut( "sgimb" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_SGIMB, Close_SGIMB )
    add_submodule ()
        set_description( N_("QuickTime Media Link") )
        add_shortcut( "qtl" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_QTL, NULL )
    add_submodule ()
        set_description( N_("Dummy IFO playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 312, Import_IFO, NULL )
    add_submodule ()
        set_description( N_("Dummy BDMV playlist") )
        set_capability( VLC_CAP_STREAM_FILTER, 312, Import_BDMV, NULL )
    add_submodule ()
        set_description( N_("iTunes Music Library playlist") )
        add_shortcut( "itml" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_iTML, NULL )
    add_submodule()
        set_description(N_("Windows Media Server metafile") )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_WMS, NULL )
    add_submodule ()
        set_description( N_("WPL playlist") )
        add_shortcut( "wpl" )
        set_capability( VLC_CAP_STREAM_FILTER, 310, Import_WPL, Close_WPL )
