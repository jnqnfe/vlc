...
#ifdef AVI_DEBUG
    msg_Dbg( s, "found chunk, fourcc: %4.4s size:%"PRIu64" pos:%"PRIu64,
...
static int AVI_ChunkRead_list( stream_t *s, avi_chunk_t *p_container )
{
    ...
    {
        msg_Warn( s, "cannot peek while reading list chunk" );
        return VLC_EGENERIC;
    }
...
#ifdef AVI_DEBUG
    msg_Dbg( s, "found LIST chunk: \'%4.4s\'",
...
#ifdef AVI_DEBUG
    msg_Dbg( s, "avih: streams:%d flags:%s%s%s%s %dx%d",
...
#ifdef AVI_DEBUG
    msg_Dbg( s, "strh: type:%4.4s handler:0x%8.8x samplesize:%d %.2ffps",
...
#ifdef AVI_DEBUG
            msg_Dbg( s, "strf: audio:0x%4.4x channels:%d %dHz %dbits/sample %dkbps",
...
#ifdef AVI_DEBUG
            msg_Dbg( s, "strf: video:%4.4s %"PRIu32"x%"PRIu32" planes:%d %dbpp",
...
#ifdef AVI_DEBUG
    msg_Dbg( s, "dmlh: dwTotalFrames %d", p_dmlh->dwTotalFrames );
#endif
    AVI_READCHUNK_EXIT( VLC_SUCCESS );
}
...
static void AVI_ChunkDumpDebug_level( stream_t *s,
                                      avi_chunk_t  *p_chk, unsigned i_level )
{
    ...
    while( p_child )
    {
        AVI_ChunkDumpDebug_level( s, p_child, i_level + 1 );
...
int AVI_ChunkReadRoot( stream_t *s, avi_chunk_t *p_root )
{
    ...
    AVI_ChunkDumpDebug_level( s, p_root, 0 );
    return VLC_SUCCESS;
}
