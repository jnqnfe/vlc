...
vlc_plugin_begin ()
    ...
    add_integer_with_range( "scaletempo-stride", 30, 1, 2000,
        N_("Stride Length (ms)"), N_("Length in milliseconds to output each stride"), true )
    ...
    add_integer_with_range( "scaletempo-search", 14, 0, 200,
        N_("Search Length (ms)"), N_("Length in milliseconds to search for best overlap position"), true )
#ifdef PITCH_SHIFTER
...
static filter_t *ResamplerCreate(filter_t *p_filter)
{
    ...
    p_resampler->p_module = vlc_module_need( p_resampler, VLC_CAP_AUDIO_RESAMPLER, NULL,
                                         false );
