...
vlc_plugin_begin ()
    ...
    add_shortcut( "stats" )
    set_capability( VLC_CAP_ENCODER, 0, OpenEncoder, NULL )
    add_submodule ()
#endif
        set_description( N_("Stats decoder function") )
        add_shortcut( "stats" )
        set_capability( VLC_CAP_VIDEO_DECODER, 0, OpenDecoder, NULL )
    add_submodule()
        set_description( N_("Stats decoder function") )
        add_shortcut( "stats" )
        set_capability( VLC_CAP_AUDIO_DECODER, 0, OpenDecoder, NULL )
    add_submodule()
        set_description( N_("Stats decoder function") )
        add_shortcut( "stats" )
        set_capability( VLC_CAP_SPU_DECODER, 0, OpenDecoder, NULL )
    add_submodule ()
        set_description( N_("Stats demux function") )
        add_shortcut( "stats" )
        set_capability( VLC_CAP_DEMUX, 0, OpenDemux, CloseDemux )
vlc_plugin_end ()
