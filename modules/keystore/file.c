...
vlc_plugin_begin()
    ...
    set_description(N_("File keystore (plaintext)"))
    set_help(N_("Secrets are stored on a file without any encryption"))
    ...
    add_submodule()
        ...
        set_help(N_("Secrets are stored encrypted on a file"))
        add_shortcut("file_crypt")
        set_capability(VLC_CAP_KEYSTORE, 1, OpenCrypt, CloseCrypt)
#endif
