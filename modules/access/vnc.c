...
vlc_plugin_begin()
    ...
    add_string( CFG_PREFIX "user", NULL, RFB_USER, NULL, false )
        change_safe()
    add_password(CFG_PREFIX "password", NULL, RFB_PASSWORD, NULL)
        change_safe()
...
static int Open( demux_t *p_demux )
{
    ...
    p_sys = vlc_obj_calloc( VLC_OBJECT(p_demux), 1, sizeof(demux_sys_t) );
