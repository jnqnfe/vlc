...
vlc_plugin_begin()
    set_shortname(N_("Mac OS X"))
    set_description(N_("Mac OS X"))
    set_capability(VLC_CAP_INTERFACE, 200, OpenIntf, CloseIntf)
    cannot_unload_broken_library()

    add_submodule()
        set_description("Mac OS X Video Output Provider")
        set_capability(VLC_CAP_VOUT_WINDOW, 100, WindowOpen, NULL)
    ...
    set_section(N_("Appearance"), 0)
        ...
        add_integer_with_range("macosx-max-volume", 125, 60, 200, VOLUME_MAX_TEXT, NULL, true)
        add_bool("macosx-large-text", false, LARGE_LISTFONT_TEXT, NULL, false)
    ...
    set_section(N_("Behavior"), 0)
        ...
        add_bool("macosx-lock-aspect-ratio", true, LOCK_ASPECT_RATIO_TEXT, NULL, true)
